...
new class extends Component {
    ...
    public ?PhaseTest $currentPhaseTest = null;
            ...
            foreach ($toSave as $q) {
                ...
                if (!$qId) {
                    $qId = Question::create([
                        ...
                        'question_text' => $this->sanitizeRichText($q['question_text']),
                        'question_type' => 'MCQ',
                    ])->id;
                    $sync = [];
                    foreach ($q['options'] as $o) {
                        $oId = $o['id'] ?: AnswerOption::create(['option_text' => $this->sanitizeRichText($o['option_text'])])->id;
                    ...
                    if ($question) {
                        ...
                        foreach ($q['options'] as $o) {
                            if (!empty($o['id'])) {
                                ...
                                if ($opt) {
                                    $opt->update(['option_text' => $this->sanitizeRichText($o['option_text'])]);
                                    $oId = $opt->id;
                                } else {
                                    $oId = AnswerOption::create(['option_text' => $this->sanitizeRichText($o['option_text'])])->id;
                                }
                            } else {
                                $oId = AnswerOption::create(['option_text' => $this->sanitizeRichText($o['option_text'])])->id;
                            }
    ...
    private function builderStateHash(array $questions): string
    {
        ...
        foreach ($questions as $q) {
            $row = [
                ...
                'question_text' => $this->sanitizeRichText((string) ($q['question_text'] ?? '')),
            ...
            foreach (($q['options'] ?? []) as $o) {
                $row['options'][] = [
                    'id' => $o['id'] ?? null,
                    'option_text' => $this->sanitizeRichText((string) ($o['option_text'] ?? '')),
    ...
    private function validateQuestionPayload(array $q, string $p, array &$dup, array &$errors): void
    {
        ...
        if (!$q['id'] && blank($this->plainText((string) $q['question_text'])))
        ...
        foreach ($q['options'] as $j => $o) {
            if (blank($this->plainText((string) $o['option_text'])))
    ...
    {
        return preg_replace('/\s+/', ' ', trim(mb_strtolower($this->plainText($t)))) ?? '';
    }
    ...
    private function sanitizeRichText(?string $html): string
    {
        $html = (string) $html;
        $html = str_replace(["\r\n", "\r"], "\n", $html);
        $html = preg_replace('/<div><br><\/div>/i', '<br>', $html) ?? $html;
        $html = preg_replace('/<(\/?)(b|strong|i|em|u|br|p|ul|ol|li|sub|sup|span|div)(?:\s+[^>]*)*>/i', '<$1$2>', $html) ?? $html;
        $html = strip_tags($html, '<b><strong><i><em><u><br><p><ul><ol><li><sub><sup><span><div>');
        $html = preg_replace('/(?:<br>\s*){3,}/i', '<br><br>', $html) ?? $html;
        $html = preg_replace('/(<(?:p|div)>\s*<\/(?:p|div)>)+/i', '', $html) ?? $html;
    ...
    public function previewRichText(?string $html, ?string $fallback = null): string
    {
        $clean = $this->sanitizeRichText($html);

        return $clean !== '' ? $clean : e((string) ($fallback ?? ''));
    }
...
<div class="mx-auto px-4 md:px-6 py-6 max-w-5xl">
    ...
    <style>
        ...
        .dark .rich-toolbar-btn {
            color: #e5e7eb;
        }

        .dark .rich-toolbar-btn:hover {
            background: #1f2937;
            border-color: #374151;
        }
    ...
    <script>
        (() => {
            ...
            function syncEditorValue(editor, immediate = false) {
                const model = editor?.dataset?.model;
                const component = getComponentForElement(editor);
                if (!model || !component || typeof component.set !== 'function') {
                    return;
                }

                const commit = () => component.set(model, normalizeEditorHtml(editor.innerHTML), true);
                if (immediate) {
                    window.clearTimeout(editorTimers.get(editor));
                    commit();
                    return;
                }

                window.clearTimeout(editorTimers.get(editor));
                editorTimers.set(editor, window.setTimeout(commit, 180));
            }

            function focusEditorByTarget(target) {
                const editor = document.querySelector(`.rich-editor[data-editor-target="${target}"]`);
                if (!editor) {
                    return null;
                }
            ...
            function insertHtmlAtCursor(html, target) {
                const editor = activeEditor?.dataset?.editorTarget === target ? activeEditor : focusEditorByTarget(target);
                if (!editor) {
                    return;
                }
            ...
            function toggleList(target, ordered = false) {
                const editor = activeEditor?.dataset?.editorTarget === target ? activeEditor : focusEditorByTarget(target);
                if (!editor) {
                    return;
                }
                ...
                const closestList = range.startContainer?.parentElement?.closest(listTag.toLowerCase());
                if (closestList && editor.contains(closestList)) {
                    const fragment = document.createDocumentFragment();
                    Array.from(closestList.children).forEach((item) => {
                        const block = document.createElement('div');
                        block.innerHTML = item.innerHTML || '<br>';
                        fragment.appendChild(block);
                    });
                    closestList.replaceWith(fragment);
                } else {
                    document.execCommand(ordered ? 'insertOrderedList' : 'insertUnorderedList', false, null);
                }

                syncEditorValue(editor, true);
            }
            ...
            function formatMathToken(token) {
                ...
                return htmlTokens[token] ?? token;
            }

            window.insertMathToken = function (token, questionUuid) {
                insertHtmlAtCursor(formatMathToken(token), `question-${questionUuid}`);
            };
            ...
            document.addEventListener('click', (event) => {
                ...
                if (actionButton) {
                    ...
                    toggleList(actionButton.dataset.editorTarget, actionButton.dataset.editorAction === 'number-list');
                    return;
                }

                const toolbarButton = event.target.closest('[data-editor-command]');
                if (!toolbarButton) {
                    return;
                }

                event.preventDefault();
                const editor = focusEditorByTarget(toolbarButton.dataset.editorTarget);
                if (!editor) {
                    return;
                }

                document.execCommand(toolbarButton.dataset.editorCommand, false, null);
                syncEditorValue(editor);
            });
