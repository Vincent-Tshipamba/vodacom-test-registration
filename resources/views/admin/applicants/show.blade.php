    ...
    $historyStatusClasses = [
        ...
        <section class="relative pt-36 pb-24">
            ...
            <div class="top-[-72px] z-10 relative flex justify-center items-center">
                <button type="button"
                    onclick="showProfilePhoto('{{ $applicant->documents->photo['url'] }}', {{ $applicant->documents->photo['id'] }}, '{{ addslashes($applicant->full_name) }}')"
                    ...
                    <div class="flex sm:flex-row flex-col justify-center gap-2 w-full">
                        ...
                        <div class="w-full">
                            <dl class="divide-y divide-gray-200 dark:divide-gray-700 text-gray-900 dark:text-white">
                                ...
                                <div class="flex flex-col py-3">
                                    <dt class="mb-1 text-gray-500 dark:text-gray-400 md:text-lg">{{ __('Coupon') }}</dt>
                                    <dd class="font-semibold text-lg">{{ $applicant->registration_code }}</dd>
                                </div>
                            </dl>
                        </div>
                    </div>
...
@section('script')
    <script>
        ...
        function updateDocumentStatus(fileId, isValid) {
            $.ajax({
                type: "PUT",
                ...
                success: function (response) {
                    showToast(isValid ? "success" : "error", response.message);
                },
                error: function () {
                    showToast(
                        "error",
        ...
        function showProfilePhoto(photoUrl, photoId, fullName) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    updateDocumentStatus(photoId, true);
                } else if (result.isDenied) {
                    updateDocumentStatus(photoId, false);
                }
            });
        }
        ...
        function showDocument(event, fileUrl, fileId, fileType, candidateId, name, isPdf) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    updateDocumentStatus(fileId, true);
                } else if (result.isDenied) {
                    updateDocumentStatus(fileId, false);
                }
            })
        }
    </script>
